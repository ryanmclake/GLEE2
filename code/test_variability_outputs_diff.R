...
map_base <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_BASE_sum.csv") %>%
  ...
  mutate(`Baseline Diffusion Predictions` =`Baseline Diffusion Predictions`*0.001*365*0.4) %>%
  na.omit(.) %>%
  ...
  #mutate(`Baseline Diffusion Predictions` = log(`Baseline Diffusion Predictions`*0.001*365*0.4+1)) %>%
  ggplot(.) +
...
map_time_low <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_TIME_sum.csv") %>%
  ...
  mutate(`Time-low Diff Predictions` =`Time-low Diff Predictions`*0.001*365*0.4) %>%
  na.omit(.) %>%
  st_as_sf(coords = c("centr_lon", "centr_lat"), crs = 4326) %>%
  st_transform("+proj=eqearth +wktext") %>%
...
map_time_high <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_TIME_sum.csv") %>%
  ...
  mutate(`Time-high Diff Predictions` =`Time-high Diff Predictions`*0.001*365*0.4) %>%
  na.omit(.) %>%
  st_as_sf(coords = c("centr_lon", "centr_lat"), crs = 4326) %>%
  st_transform("+proj=eqearth +wktext") %>%
...
map_space_low <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_SPACE_sum.csv") %>%
  ...
  mutate(`Space-low Diff Predictions` =`Space-low Diff Predictions`*0.001*365*0.4) %>%
  na.omit(.) %>%
  st_as_sf(coords = c("centr_lon", "centr_lat"), crs = 4326) %>%
  st_transform("+proj=eqearth +wktext") %>%
...
map_space_high <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_SPACE_sum.csv") %>%
  ...
  mutate(`Space-high Diff Predictions` =`Space-high Diff Predictions`*0.001*365*0.4) %>%
  na.omit(.) %>%
  st_as_sf(coords = c("centr_lon", "centr_lat"), crs = 4326) %>%
  st_transform("+proj=eqearth +wktext") %>%
...
map_param_low <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_PARAM_sum.csv") %>%
  ...
  mutate(`Param-low Diff Predictions` =`Param-low Diff Predictions`*0.001*365*0.4) %>%
  na.omit(.) %>%
  st_as_sf(coords = c("centr_lon", "centr_lat"), crs = 4326) %>%
  st_transform("+proj=eqearth +wktext") %>%
...
map_param_high <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_PARAM_sum.csv") %>%
  ...
  mutate(`Param-high Diff Predictions` =`Param-high Diff Predictions`*0.001*365*0.4) %>%
  na.omit(.) %>%
  st_as_sf(coords = c("centr_lon", "centr_lat"), crs = 4326) %>%
  st_transform("+proj=eqearth +wktext") %>%
...
map_model_low <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_MODEL_sum.csv") %>%
  ...
  mutate(`Model-low Diff Predictions` =`Model-low Diff Predictions`*0.001*365*0.4) %>%
  na.omit(.) %>%
  st_as_sf(coords = c("centr_lon", "centr_lat"), crs = 4326) %>%
  st_transform("+proj=eqearth +wktext") %>%
...
map_model_high <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_MODEL_sum.csv") %>%
  ...
  mutate(`Model-high Diff Predictions` =`Model-high Diff Predictions`*0.001*365*0.4) %>%
  na.omit(.) %>%
  st_as_sf(coords = c("centr_lon", "centr_lat"), crs = 4326) %>%
  st_transform("+proj=eqearth +wktext") %>%
...
line <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/Global_diffusion_variability_BASE_mean.csv") %>%
  mutate(`Baseline Diffusion Predictions` = log(`Baseline Diffusion Predictions`+1)) %>%
  mutate(centr_lat = ceiling(centr_lat)) %>%
  group_by(centr_lat) %>%
  summarise(`Baseline Diffusion Predictions` = mean(`Baseline Diffusion Predictions`))

plot(line$`Baseline Diffusion Predictions`, line$centr_lat, type = "l", col = "red")
...
area_hexes_avg <- area_hexes %>%
  ...
  group_by(index) %>%
  mutate(bin_count = n()) %>%
  summarise(`Baseline Diffusion Predictions` = mean(`Baseline Diffusion Predictions`, na.rm = TRUE),
...
flux_map <-
  ...
  scale_color_gradient(low="blue",
                       high="red", space ="Lab", na.value="black",
                       name = "**Diffusion Flux** <br>log(mg CH4 m-1 yr-1)") +
