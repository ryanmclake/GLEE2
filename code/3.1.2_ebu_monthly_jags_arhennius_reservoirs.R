...
sd_values = c(unique(ebu_error2$se))
...
model {  
  ...
  ### MONTHLY TIMESTEP ###
  ebu_base_temp <- base %>% select(ch4_ebu, temp_for_model_K,country, waterbody_type, year, month, tot_sampling_events) %>%
...
res_proportion_output = as.data.frame(do.call(rbind, out_big))
res_parameters_output = as.data.frame(do.call(rbind, out_coefficients))

res_proportions <- res_proportion_output %>% rename(se = se) %>% left_join(., ebu_error2, by = "se") %>%
  filter(row_names != "sum_var")

res_parameters <- res_parameters_output %>% rename(se = prediction_level) %>% left_join(., ebu_error2, by = "se")

res_coefficient <- res_proportion_output %>% rename(se = se) %>% left_join(., ebu_error2, by = "se") %>%
  filter(row_names != "sum_var")

res_total_var <- res_proportion_output %>% rename(se = se) %>% left_join(., ebu_error2, by = "se") %>%
  filter(row_names == "sum_var")
...
res_proportions%>%
  ...
  theme_bw()+
  #labs(title = "A: Partitioned uncertainty with increasing sample size for lakes")+
  labs(title = "A: Partitioned uncertainty with increasing sample size for reservoirs")+
...
res_validate <- cbind(prediction_output_long, ebu_base_temp$temp_for_model_C, ebu_base_temp$ch4_ebu) %>%
