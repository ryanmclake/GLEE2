...
model {  
  ...
  for(i in 1:nchain){
    init[[i]] <- list(sd.pro = runif(1, 1, 2),
                      ...
                      a = runif(1, mean(ebu_coefficients$omega), 5),
...
lake_proportion_output = as.data.frame(do.call(rbind, out_big))
lake_parameters_output = as.data.frame(do.call(rbind, out_coefficients))

lake_proportions <- lake_proportion_output %>% rename(se = se) %>% left_join(., ebu_error2, by = "se") %>%
  filter(row_names != "sum_var")

lake_parameters <- lake_parameters_output %>% rename(se = prediction_level) %>% left_join(., ebu_error2, by = "se")

lake_coefficient <- lake_proportion_output %>% rename(se = se) %>% left_join(., ebu_error2, by = "se") %>%
  filter(row_names != "sum_var")

lake_total_var <- lake_proportion_output %>% rename(se = se) %>% left_join(., ebu_error2, by = "se") %>%
  filter(row_names == "sum_var")
...
lake_validate <- cbind(prediction_output_long, ebu_base_temp$temp_for_model_C, ebu_base_temp$ch4_ebu) %>%
