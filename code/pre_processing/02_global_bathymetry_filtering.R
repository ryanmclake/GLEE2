...
### and calculates the area of the lake with depths "LESS THAN" 12 meters. A proxy for ebullition emissions following XXX. 
...
extract_littoral <- function(bathy){
  ...
  lit <- stars::read_stars(paste0("/Volumes/SeagateBackupPlusDrive/Bathymetry_Rasters/",bathy,"")) %>%
    ...
    dplyr::mutate(hylak_id = sub(".*/", "", bathy),
      ...
      utils::write.table(., file = paste0("./data/littoral_area/global_littoral_area_12.csv"),
                  append = T,
                  row.names = F,
                  col.names = !file.exists("./data/littoral_area/global_littoral_area_12.csv"))
