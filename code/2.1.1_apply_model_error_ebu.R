...
model_error_a <- sqrt(deviance(mean_ebu_MFOA))
...
FOA_mean_error_low_lake <- function(x){
  predicted_ebu_rate = (coefficients_a[1,1] * coefficients_a[1,2]^(x-20)) - model_error_a
  return(predicted_ebu_rate)
}
...
model_error_b <- sqrt(deviance(mean_ebu_SOA_temp_soil_threshold))
...
SOA_error_low <- function(x, p){
  predicted_ebu_rate = coefficients_b[1,1] * exp((coefficients_b[1,2]*x) * (p/coefficients_b[1,3]+p)) + model_error_b
  return(predicted_ebu_rate)
}
...
coefficients_c <- rbind(LM_mean_ebu$coefficients)
model_error_c <- sqrt(deviance(LM_mean_ebu))


LM_mean <- function(x, p){
  predicted_ebu_rate = coefficients_c[1,1] + p*coefficients_c[1,2]
  return(predicted_ebu_rate)
}

LM_error_low <- function(x, p){
  predicted_ebu_rate = coefficients_c[1,1] + p*coefficients_c[1,2] - model_error_c
  return(predicted_ebu_rate)
}
...
coefficients_d <- rbind(mean_ebu_MFOA_res$m$getPars())
model_error_d <- sqrt(deviance(mean_ebu_MFOA_res))

FOA_mean_res <- function(x){
  predicted_ebu_rate = (coefficients_d[1,1] * coefficients_d[1,2]^(x-20))
  return(predicted_ebu_rate)
}

FOA_mean_error_low_res <- function(x){
  predicted_ebu_rate = (coefficients_d[1,1] * coefficients_d[1,2]^(x-20)) - model_error_d
  return(predicted_ebu_rate)
}
...
coefficients_e <- rbind(mean_ebu_SOA_temp_soil_threshold_res$m$getPars())
model_error_e <- sqrt(deviance(mean_ebu_SOA_temp_soil_threshold_res))


SOA_mean_res <- function(x, p){
  predicted_ebu_rate = coefficients_e[1,1] * exp((coefficients_e[1,2]*x) * (p/coefficients_e[1,3]+p))
  return(predicted_ebu_rate)
}

SOA_error_low_res <- function(x, p){
  predicted_ebu_rate = coefficients_e[1,1] * exp((coefficients_e[1,2]*x) * (p/coefficients_e[1,3]+p)) + model_error_e
  return(predicted_ebu_rate)
}
...
coefficients_f <- rbind(LM_mean_ebu_res$coefficients)
model_error_f <- sqrt(deviance(LM_mean_ebu_res))


LM_mean_res <- function(x, p){
  predicted_ebu_rate = coefficients_f[1,1] + p*coefficients_f[1,2]
  return(predicted_ebu_rate)
}

LM_error_low_res <- function(x, p){
  predicted_ebu_rate = coefficients_f[1,1] + p*coefficients_f[1,2] - model_error_f
  return(predicted_ebu_rate)
}
...
calc_global_variance <- function(country){
  ...
  d_model <- d %>%
    ...
    mutate(FOA_mean_ebu = ifelse(waterbody_type == 1, mapply(FOA_mean, x=mean_temp_k-273.15), mapply(FOA_mean_res, x=mean_temp_k-273.15)),
           FOA_error_high = ifelse(waterbody_type == 1, mapply(FOA_mean_error_high, x=mean_temp_k-273.15), mapply(FOA_mean_error_high_res, x=mean_temp_k-273.15)),
           FOA_error_low = ifelse(waterbody_type == 1, mapply(FOA_mean_error_low, x=mean_temp_k-273.15), mapply(FOA_mean_error_low_res, x=mean_temp_k-273.15)),
           FOA_error_low = ifelse(FOA_error_low<=0,0,FOA_error_low),
           'date' = lubridate::make_date(year = year, month = month)) %>%
    ungroup(.) %>%
    select(-year, -month) %>%
    utils::write.table(., file = paste0("/Users/ryanmcclure/Documents/GLEE2.1/output/global_ebullition_model_variance.csv"),
                       append = T,
                       row.names = F,
                       col.names = !file.exists("/Users/ryanmcclure/Documents/GLEE2.1/output/global_ebullition_model_variance.csv"))
}
...
measurement <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/global_ebullition_model_variance.csv") %>%
  filter(total_km2 < 10000) %>%
  group_by(lat, lon, hylak_id) %>%
  summarize(FOA_mean_ebu = median(FOA_mean_ebu),
            FOA_error_high = median(FOA_error_high),
            FOA_error_low = median(FOA_error_low)) %>%
  mutate(error_diff = FOA_error_high-FOA_error_low) %>%
  filter(lon < 60) %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326) %>%
  st_transform("+proj=eqearth +wktext")

boxplot_model <- vroom::vroom("/Users/ryanmcclure/Documents/GLEE2.1/output/global_ebullition_model_variance.csv") %>%
  filter(total_km2 < 10000) %>%
  group_by(lat, lon, hylak_id) %>%
  summarize(FOA_mean_ebu = median(FOA_mean_ebu),
            FOA_error_high = median(FOA_error_high),
            FOA_error_low = median(FOA_error_low)) %>%
  ungroup(.) %>%
  select(hylak_id, FOA_error_high, FOA_error_low) %>%
  reshape2::melt(., id = "hylak_id")

ggplot(boxplot_model, aes(x=variable, y=value)) + 
  geom_boxplot(outlier.colour="red", outlier.shape=NA,
               outlier.size=0)

boxes <- rbind(boxplot, boxplot_model)
...
ggplot()+
  geom_sf(data = measurement, pch = 16, size = 0.2, aes(color = error_diff))+
  scale_color_gradient(low="darkblue",
                       high="red", space ="Lab", na.value="grey",
                       name = "**Ebullitive Methane Flux ** ",
                       limits = c(0,3000))

ggplot()+
  geom_sf(data = measurement, pch = 16, size = 0.2, aes(color = FOA_time_low_ebu))+
  scale_color_gradient(low="darkblue",
                       high="red", space ="Lab", na.value="grey",
                       name = "**Ebullitive Methane Flux ** ",
                       limits = c(0,1500))
...
ggplot()+
  geom_sf(data = measurement, pch = 16, size = 0.2, aes(color = FOA_time_high_ebu))+
  scale_color_gradient(low="darkblue",
                       high="red", space ="Lab", na.value="grey",
                       name = "**Ebullitive Methane Flux ** ",
                       limits = c(0,1500))
...
ggplot()+
  geom_sf(data = measurement, pch = 16, size = 0.2, aes(color = FOA_space_low_ebu))+
  scale_color_gradient(low="darkblue",
                       high="red", space ="Lab", na.value="grey",
                       name = "Ebullition calibrated off of lower qunatile",
                       limits = c(0,1500))

ggplot()+
  geom_sf(data = measurement, pch = 16, size = 0.2, aes(color = FOA_space_high_ebu))+
  scale_color_gradient(low="darkblue",
                       high="red", space ="Lab", na.value="grey",
                       name = "Ebullition Assuming Maximimum Spatial Variability",
                       limits = c(0,1500))


ggplot()+
  geom_sf(data = measurement, pch = 16, size = 0.2, aes(color = time_diff))+
  scale_color_gradient(low="darkblue",
                       high="red", space ="Lab", na.value="grey",
                       name = " Temporal Variability")

ggplot()+
  geom_sf(data = measurement, pch = 16, size = 0.2, aes(color = space_diff))+
  scale_color_gradient(low="darkblue",
                       high="red", space ="Lab", na.value="grey",
                       name = " Temporal Variability")
