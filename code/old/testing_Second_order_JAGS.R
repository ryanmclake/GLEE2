...
#ebu_error <- ebu_error %>% mutate(sd = ifelse(sd == 0, 0.1, sd))
...
for(k in 1:length(sd_values)){
  ...
  ahrennius_soil_ebu_model = ("arhennius_soil_ebu_model.txt")
  jagsscript = cat("
model {  
  ...
  ### MONTHLY TIMESTEP ###
  ebu_base_temp <- base2 %>% select(ch4_ebu, temp_for_model_K,country, waterbody_id, year, month, tot_sampling_events,T_OC) %>%
    na.omit(.) %>%
  ...
  j.model   <- jags.model(file = ahrennius_soil_ebu_model,
                          data = jags.data,
                          inits = init,
                          n.chains = 3)
  
  eval_ebu  <- coda.samples(model = j.model,
                            variable.names = c("sd.pro","A","a","b", "sd.obs"),
...
proportions <- proportion_output %>% rename(se = se) %>% left_join(., ebu_error2, by = "se") %>%
  filter(row_names != "sum_var")

parameters <- parameters_output %>% rename(se = prediction_level) %>% left_join(., ebu_error2, by = "se")

coefficient <- proportion_output %>% rename(se = se) %>% left_join(., ebu_error2, by = "se") %>%
  filter(row_names != "sum_var")

total_var <- proportion_output %>% rename(se = se) %>% left_join(., ebu_error2, by = "se") %>%
  filter(row_names == "sum_var")
