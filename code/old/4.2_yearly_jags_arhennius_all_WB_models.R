...
model {  
  ...
  ### MONTHLY TIMESTEP ###
  ebu_base_temp <- base %>% 
    group_by(year,country,waterbody_id,tot_sampling_events) %>%
    summarize(ch4_ebu = mean(ch4_ebu),
              temp_for_model_K = mean(temp_for_model_K),
              tot_sampling_events = sum(tot_sampling_events)) %>%
    select(ch4_ebu, temp_for_model_K,country, waterbody_id, year, tot_sampling_events) %>%
    na.omit(.) %>%
