...
base <- read_csv("./data/GLEE_data_with_GLCP_link.csv") %>%
...
error_ebu <- base %>% select(waterbody_id, num_months_sampled, num_sites_sampled, tot_sampling_events, ch4_ebu) %>% 
    na.omit(.) %>%
...
error_diff <- base %>% select(waterbody_id, num_months_sampled, num_sites_sampled, tot_sampling_events, ch4_diff) %>% 
  na.omit(.) %>%
  group_by(waterbody_id) %>%
