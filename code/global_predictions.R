...
d <- vroom::vroom(paste0("/Users/ryanmcclure/Documents/GLEE2.1/data/countries/Japan.csv"), col_names = F, delim = ",") %>%
  ...
  mutate(FOA_mean_ebullition = ifelse(waterbody_type == 1, mapply(FOA_mean_lake, x=mean_temp_k-273.15), mapply(FOA_mean_res, x=mean_temp_k-273.15)),
         ...
         FOA_time_high_ebu = ifelse(waterbody_type == 1, mapply(FOA_time_low_lake, x=mean_temp_k-273.15), mapply(FOA_time_high_res, x=mean_temp_k-273.15)),
         FOA_space_low_ebu = ifelse(waterbody_type == 1, mapply(FOA_time_low_lake, x=mean_temp_k-273.15), mapply(FOA_space_low_res, x=mean_temp_k-273.15)),
         FOA_space_high_ebu = ifelse(waterbody_type == 1, mapply(FOA_time_low_lake, x=mean_temp_k-273.15), mapply(FOA_space_high_res, x=mean_temp_k-273.15)),
         ...
         FOA_mean_ebullition = ifelse(ice_cover_mean > 60, 0, FOA_mean_ebullition),
         FOA_time_low_ebu = ifelse(ice_cover_mean > 60, 0, FOA_time_low_ebu),
         FOA_time_high_ebu = ifelse(ice_cover_mean > 60, 0, FOA_time_high_ebu),
         FOA_space_low_ebu = ifelse(ice_cover_mean > 60, 0, FOA_space_low_ebu),
         FOA_space_high_ebu = ifelse(ice_cover_mean > 60, 0, FOA_space_high_ebu),
         FOA_mean_error_low_ebu = ifelse(ice_cover_mean > 60, 0, FOA_mean_error_low_ebu),
         FOA_mean_error_high_ebu = ifelse(ice_cover_mean > 60, 0, FOA_mean_error_high_ebu),
         FOA_coefficient_low_ebu = ifelse(ice_cover_mean > 60, 0, FOA_coefficient_low_ebu),
         FOA_coefficient_high_ebu = ifelse(ice_cover_mean > 60, 0, FOA_coefficient_high_ebu),
  ...
  ungroup(.) %>%
  select(-year, -month) %>%
  utils::write.table(., file = paste0("/Users/ryanmcclure/Documents/GLEE2.1/output/J_ebullition_variability.csv"),
                     append = T,
                     row.names = F,
                     col.names = !file.exists("/Users/ryanmcclure/Documents/GLEE2.1/output/J_ebullition_variability.csv"))
